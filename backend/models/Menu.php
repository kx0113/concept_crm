...
class Menu extends \yii\db\ActiveRecord
{
    ...
    public function  getAllMenu(){
        $menu = Yii::$app->db->createCommand("SELECT * FROM `kx_menu` WHERE parent='0' ")->queryAll();
        return $menu;
    }
    //获取所有菜单列表
    public function  getMenuList(){
        $menu = Yii::$app->db->createCommand("SELECT * FROM `kx_menu` ORDER BY sort ASC ")->queryAll();
    ...
    public function getRouteById($id){
        $router = Yii::$app->db->createCommand("SELECT * FROM `kx_menu` WHERE id='$id'")->queryOne();
        return $router['route'];
    }
